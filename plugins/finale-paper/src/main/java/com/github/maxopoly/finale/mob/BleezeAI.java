...
public class BleezeAI {
    ...
    public void start() {
        Plugin finalePlugin = Finale.getPlugin();
        // TODO is global scheduler safe here?
        Bukkit.getGlobalRegionScheduler().runAtFixedRate(finalePlugin, (task) -> {
            for (ActiveMob mob : MythicBukkit.inst().getMobManager().getActiveMobs()) {
                mob.getEntity().getBukkitEntity().getScheduler().execute(finalePlugin,
                    () -> {
